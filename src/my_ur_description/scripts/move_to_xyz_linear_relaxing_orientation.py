...
class UR12eCartesianController(Node):
    ...
    def move_to_xyz_linear(self, x, y, z):
        ...
        # FIX: Changed from 0.0 to 5.0 to allow for smoother joint transitions
        # This stops the "crazy" spinning when moving between nearby points.
        ...
        if res and res.fraction > 0.95:
            self.get_logger().info(f"Path success ({res.fraction*100:.1f}%). Executing...")
            ...
            # Ensure the robot finishes the move before the script continues
            ...
            self.get_logger().error(f"Cartesian path failed! Only {res.fraction*100:.1f}% planned.")
            return False
...
def main():
    ...
    # Linear sequence
    # Moving vertically (Z-axis) is usually very safe for Cartesian paths.
    # Moving horizontally (Y-axis) is where "jumps" usually happen.
    ...
    bot.get_logger().info("Sequence complete.")
    bot.destroy_node()
    rclpy.shutdown()
