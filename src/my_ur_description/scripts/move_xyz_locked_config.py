...
class UR12eLockedController(Node):
    def __init__(self):
        super().__init__('ur12e_locked_controller')
    ...
    def move_to_xyz(self, x, y, z):
        # 1. Wait for joint state to ensure we have a "Seed"
        ...
        # 2. Try Cartesian first (Strict Straight Line)
        req = GetCartesianPath.Request()
        req.header.frame_id = "world"
        req.group_name = "ur_manipulator"
        req.waypoints = [target_pose]
        req.max_step = 0.01          
        req.jump_threshold = 0.0 # Keep it strict for Cartesian
        ...
        if res and res.fraction > 0.95:
            self.get_logger().info(f"Straight line to {x}, {y}, {z} possible. Executing...")
            return self.execute_traj(res.solution)

        # 3. Fallback: Seeded IK (Forces the same Shoulder/Elbow config)
        self.get_logger().warn("Linear path impossible. Finding same-config joint solution...")
        ...
        self.get_logger().error("Target unreachable in this configuration!")
        return False
...
def main():
    rclpy.init()
    bot = UR12eLockedController()
    
    targets = [
        (0.75, 0.0, 1.5), (0.75, 0.0, 1.2), (0.75, 0.0, 1.5),
        (0.75, 0.1, 1.5), (0.75, 0.1, 1.2)
    ]
